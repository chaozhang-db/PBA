...
import experiments.util.CountBasedThroughputExperiment;
import experiments.util.DataLoader;
import experiments.util.ThroughputExperiment;
import experiments.util.TimeBasedThroughputExperiment;
...
public class Throughput {
    ...
    private static final String pathToRealData = DataLoader.DEBS_12, pathToSyntheticData = DataLoader.SYNTHETIC_DATA, pathToPoissonStream = DataLoader.POISSON_STREAM;
    ...
    private static void throughputExperiments() {
        setupTimeBasedThroughputExperimentsWithPoissonStream();
        setupCountBasedThroughputExperiments();
        setupTimeBasedThroughputExperiments();
        try {
            PrintWriter printWriter = new PrintWriter(new FileOutputStream((pathToThroughputExperimentsResults)));
    ...
    private static void setupCountBasedThroughputExperiments() {
        ...
        for (boolean useRealData : isRealData) {
            if (useRealData) {
                ...
                stream = DataLoader.loadSyntheticDataWithoutTimeStamps(pathToSyntheticData, sizeOfSyntheticDataSet);
            ...
            for (String algo : algos) {
                ...
                if (algo.equalsIgnoreCase("PBA")) ParallelBoundaryAggregator.threadPool.shutdown();
            }
        }
    }
    ...
    private static void setupTimeBasedThroughputExperiments() {
        ...
        for (String algo : algos) {
            if (algo.equalsIgnoreCase("PBA")) ParallelBoundaryAggregator.threadPool = Executors.newCachedThreadPool();
    ...
    private static void setupTimeBasedThroughputExperimentsWithPoissonStream() {
        System.out.println("start experiments for time-based windows");

        Queue<IntTuple> stream = DataLoader.load(pathToPoissonStream, true, 400_000_000, 1);
        IntTuple[] outputStream = new IntTuple[stream.size()];

        System.out.println("Poisson stream size: " + stream.size());
        ...
        for (String algo : algos) {
            ...
            for (Duration range : ranges)
                runThroughputExperiment(algo, range, Duration.ofMillis(10), stream, outputStream, true, false);
            ...
            for (Duration slide : slides)
                runThroughputExperiment(algo, Duration.ofHours(1), slide, stream, outputStream, false, false);
